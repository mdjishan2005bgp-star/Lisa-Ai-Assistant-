 ...
 */
export function resetNexaSession() {
  chatSession = null;
}
 ...
 */
export async function getNexaResponse(prompt: string, history: { sender: "user" | "nexa", text: string }[] = []): Promise<string> {
  try {
    if (!chatSession) {
...
export async function getNexaAudio(text: string): Promise<string | null> {
  try {
